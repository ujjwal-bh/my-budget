...
import { ArrowDownRight, ArrowUpRight, BarChart3, CreditCard, LayoutDashboard, Pencil, Plus, Target, Trash2, Wallet, X } from "lucide-react";
...
type Page = "Overview" | "Transactions" | "Reports" | "Spending goal" | "Categories";
const navItems: { label: Page; icon: typeof LayoutDashboard }[] = [
  { label: "Overview", icon: LayoutDashboard }, { label: "Transactions", icon: CreditCard }, { label: "Reports", icon: BarChart3 }, { label: "Spending goal", icon: Target }, { label: "Categories", icon: Wallet },
];
...
export default function Home() {
  ...
  async function saveTransaction(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!session || !merchant || !amount || !category) return;
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark"><Wallet size={18} /></span><span>pocketwise</span></div><nav className="primary-nav"><span className="nav-label">Workspace</span>{navItems.map(({ label, icon: Icon }) => <button key={label} className={`nav-item ${page === label ? "nav-item-active" : ""}`} onClick={() => setPage(label)}><Icon size={18} /><span>{label}</span></button>)}</nav><div className="sidebar-bottom"><button className="sign-out-button" onClick={() => void supabase.auth.signOut()}>Sign out</button></div></aside>
    <section className="content-area"><header className="topbar"><div className="breadcrumb"><span>Workspace</span><span>/</span><strong>{page}</strong></div><span className="user-avatar small">{displayName.slice(0, 2).toUpperCase()}</span></header><div className="page-content">
      {page === "Overview" && <Overview month={selectedMonth} setMonth={setSelectedMonth} spent={spent} income={income} previousSpent={previousSpent} difference={difference} goal={spendingGoal} progress={goalProgress} transactions={monthTransactions} onAdd={openCreate} onEdit={openEdit} onDelete={deleteTransaction} onViewTransactions={() => setPage("Transactions")} />}
    ...
    {showForm && <TransactionForm editing={editing} merchant={merchant} amount={amount} category={category} type={type} categories={categories} setMerchant={setMerchant} setAmount={setAmount} setCategory={setCategory} setType={setType} onClose={() => setShowForm(false)} onSubmit={saveTransaction} />}
  </main>;
}
...
function AuthScreen() { const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [error, setError] = useState(""); async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const { error: authError } = await supabase.auth.signInWithPassword({ email, password }); setError(authError?.message ?? ""); } return <main className="auth-shell"><section className="auth-card"><div className="brand auth-brand"><span className="brand-mark"><Wallet size={18} /></span><span>pocketwise</span></div><p className="eyebrow">Your money, in focus</p><h1>Welcome back</h1><form onSubmit={submit} className="auth-form"><label>Email address<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>{error && <p className="auth-error">{error}</p>}<button className="add-button auth-submit">Sign in</button></form></section></main> }
